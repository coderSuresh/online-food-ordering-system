...
if (!isset($whichPage) || (isset($whichPage) && $whichPage != "customer")) {
    ...
    $sql_order = "SELECT id FROM orders group by orders.track_id";
    $sql_temp_customer = "SELECT names FROM customer";
    $sql_order_canceled = "SELECT o_r_id FROM reject_reason inner join orders on reject_reason.track_id = orders.track_id group by orders.track_id";
    ...
    switch ($filter_option) {
        case "today":
            ...
            $sql_order = "SELECT id FROM orders WHERE DATE(date) = CURDATE() group by orders.track_id";
            $sql_temp_customer = "SELECT names FROM customer WHERE DATE(date) = CURDATE()";
            $sql_order_canceled = "SELECT o_r_id FROM reject_reason inner join orders on reject_reason.track_id = orders.track_id WHERE DATE(orders.date) = CURDATE() group by orders.track_id";
        ...
        case "yesterday":
            ...
            $sql_order = "SELECT id FROM orders WHERE DATE(date) = CURDATE() - INTERVAL 1 DAY group by orders.track_id";
            $sql_temp_customer = "SELECT names FROM customer WHERE DATE(date) = CURDATE() - INTERVAL 1 DAY";
            $sql_order_canceled = "SELECT o_r_id FROM reject_reason inner join orders on reject_reason.track_id = orders.track_id WHERE DATE(orders.date) = CURDATE() - INTERVAL 1 DAY group by orders.track_id";
        ...
        case "last-week":
            ...
            $sql_order = "SELECT id FROM orders WHERE YEARWEEK(date) = YEARWEEK(CURDATE() - INTERVAL 1 WEEK) group by orders.track_id";
            $sql_temp_customer = "SELECT names FROM customer WHERE YEARWEEK(date) = YEARWEEK(CURDATE() - INTERVAL 1 WEEK)";
            $sql_order_canceled = "SELECT o_r_id FROM reject_reason inner join orders on reject_reason.track_id = orders.track_id WHERE YEARWEEK(orders.date) = YEARWEEK(CURDATE() - INTERVAL 1 WEEK) group by orders.track_id";
        ...
        case "last-month":
            ...
            $sql_order = "SELECT id FROM orders WHERE MONTH(date) = MONTH(CURDATE() - INTERVAL 1 MONTH) group by orders.track_id";
            $sql_temp_customer = "SELECT names FROM customer WHERE MONTH(date) = MONTH(CURDATE() - INTERVAL 1 MONTH)";
            $sql_order_canceled = "SELECT o_r_id FROM reject_reason inner join orders on reject_reason.track_id = orders.track_id WHERE MONTH(orders.date) = MONTH(CURDATE() - INTERVAL 1 MONTH) group by orders.track_id";
        ...
        case "custom":
            ...
            $sql_order = "SELECT id FROM orders WHERE date >= '$start_date 00:00:00' AND  date <= '$end_date 23:59:59' group by orders.track_id";
            $sql_temp_customer = "SELECT names FROM customer WHERE date >= '$start_date 00:00:00' AND date <= '$end_date 23:59:59'";
            $sql_order_canceled = "SELECT o_r_id FROM reject_reason inner join orders on reject_reason.track_id = orders.track_id WHERE orders.date >= '$start_date 00:00:00' AND orders.date <= '$end_date 23:59:59' group by orders.track_id";
